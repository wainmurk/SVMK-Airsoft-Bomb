...
#define volt_5v 5.0   //Рельное напряжение на плате по линии 5в (для точности индикатора заряда)
...
const uint8_t gameover[] = { 0xa1, 0x70, 0x79, 0x20, 0xb7, 0x61, 0xba, 0x69, 0xbd, 0xc0, 0x65, 0xbd, 0x6f, 0x00 };                         //Гру закінчено
...
const uint8_t gamestarting[] = { 0x48, 0x61, 0xc0, 0x61, 0xbf, 0x6f, 0xba, 0x20, 0xb4, 0x70, 0xb8, 0x00 };                                 //Начаток гри
...
void setup() {
  ...
  if (GAMEMINUTES < 0) GAMEMINUTES = 5;
  ...
  delay(1500);
  lcd.clear();
  input_voltage = (analogRead(A6) * volt_5v) / 1023.0;  //вольты
